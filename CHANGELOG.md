...
- refactor(tests): **`review-police.test.ts` is split into `tests/review-police/`, under the file cap.**
  ...
  same assertions — redistributed across eight files by their existing describe groups, over five
  ...
  self-proof cases that assert this now prove it for the new layout, and a guard test reads every
  sibling file and fails if one takes `test` or `it` from `bun:test` instead — the wrapper is an
  import now, not a lexical shadow, so losing it would otherwise be silent. The fixture's mutable forge
