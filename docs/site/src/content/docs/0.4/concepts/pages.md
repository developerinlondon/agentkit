---
title: Pages
description: The self-hosted artifact publisher — how a page gets a stable URL,
  and the three secrets that keep rotation from orphaning it.
sidebar:
  order: 6
slug: 0.4/concepts/pages
---
...
The skill publishes end to end without asking about slug, template or mechanics — [publish a page](/docs/0.4/cookbook/publish-a-page/) walks one through. The one carve-out is
...
[`pages-police`](/docs/0.4/reference/hooks/) refuses raw `curl`, `wget`, `httpie` and `xh` writes to the
