...
`core` always installs and cannot be deselected. `memory` is the brain-vault learning loop —
`reflect`, `meditate` and `ruminate`, plus two hooks that inject the vault index at session start
and keep it rebuilt after writes. `product` is `product-intelligence` and
...
./agentkit/install.sh --global --with memory          # core + the memory kit
...
Every non-explicit opt-in kit also has a direct install shim under `kits/<id>` — one copy-paste
line that runs the normal bootstrap with that kit preselected, so a kit's README can advertise
itself without explaining the installer:

```sh
curl -fsSL https://raw.githubusercontent.com/developerinlondon/agentkit/main/kits/memory | bash
curl -fsSL https://raw.githubusercontent.com/developerinlondon/agentkit/main/kits/product | bash
```

The shims go through `bootstrap.sh` and `install.sh` unchanged, so the recorded kit selection and
...
:::note[Type the flag to get the gate]
The review machinery is opt-in: neither `--all` nor the picker installs it. `--with
adversarial-review` is the only way in.
:::
