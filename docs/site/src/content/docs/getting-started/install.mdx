...
Piped stdin is not a terminal, so the optional-kit question never fires on this path — it
installs `core` only unless you pass `--with <kit>`.
...
:::note[Run this line yourself, before the install]
...
| `--all`               | Select every declared kit *except* those marked explicit in the manifest.                                                  |
...
```sh
ls ~/.agentkit/skills      # 13 core; 15 with --all; 16 with --all --with adversarial-review
cat ~/.agentkit/kits     # the remembered selection
...
- [Skill kits](/docs/getting-started/skill-kits/) — what is optional, and what the wizard
