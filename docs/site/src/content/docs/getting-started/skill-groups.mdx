---
title: Skill groups
description: How skills are partitioned into groups, and exactly when the installer asks.
sidebar:
  order: 2
---
...
The groups declared today, read out of the manifest at build time:

<GroupTable />

`core` always installs and cannot be deselected. `product` is `product-intelligence` and
`product-review`. `strict-review` is `adversarial-review`, plus the merge gate's hooks, tools and
the `evidence-gated-review` instruction file. `advisory-review` carries no skill at all — it gates
one instruction file, `review-discipline.md`, which asks for a non-authoring reviewer pass before a
merge. A group with no skills publishes no Claude Code plugin, because skills are the whole payload
of a generated group plugin.

The two review groups are the kit's **two review modes**, and they are independent: enable either,
both, or neither. `advisory-review` asks; `strict-review` refuses.
...
./agentkit/install.sh --global --with advisory-review --with strict-review   # both review modes
```
