...
with the `review-discipline` group as `instructions/review-discipline.md`. Nothing enforces it, and
it is opt-in: an instruction is concatenated into every prompt, so a harness that already mandates a
