...
Re-running the installer *is* the upgrade. There is no version file, no install manifest and no
...
| `memory`, `product`, `clickup` | removes managed skills, hooks, settings entries, prompts, and plugins |
