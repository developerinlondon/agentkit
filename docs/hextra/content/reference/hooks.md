...
| `editor-police`     | a `git commit` in a repo listed under `editor-police.repos` until the session has said who is at the keyboard (`wiki-editor set`), and again unless that very commit carries the person as an `Edited-by` trailer written out in full. The command is tokenised with shell quoting, so the repo is the one the commit targets (`-C`, `--git-dir`, `cd` before it, a worktree of a listed clone, a `bash -c` string) and a path inside a message is not one. Refuses with `UNCHECKED` rather than allowing when it cannot judge: `awk` or `jq` missing, or more than 1500 statements in one call                                                                                                                                                                                                  |
...
| `editor-police`     | installed, but inert without listed repos           | a repo glob under `editor-police.repos` |
