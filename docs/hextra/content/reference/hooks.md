...
| `prose-police`      | AI writing tells in the ADDED prose of a markdown or text write — stock vocabulary (delve, tapestry, leverage), significance inflation ("plays a crucial role"), negative parallelism ("not just X, but Y"), chatbot filler, and em-dash density over the limit. Fenced and inline code are exempt, as are changelogs and the artifacts that teach the patterns                                                                        |
