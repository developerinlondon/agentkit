...
| `issue-police`      | filing an issue whose body does not carry a `Disposition:` line. It checks presence, never the answer                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `prose-police`      | AI writing tells in the ADDED prose of a markdown or text write, and in the inline `--body`/`--description`/`--title`/`--notes` text of a `gh`/`glab` command (the Bash arm denies; bodies passed by file are caught when the file is written): stock vocabulary (`delve`, `tapestry`, `leverage`), significance inflation (`plays a crucial role`), negative parallelism (`not just X, but Y`), chatbot filler, and em-dash density over the limit. Fenced and inline code are exempt, as are changelogs and the artifacts that teach the patterns |
...
`AGENTKIT_SKIP_HOOKS` is honoured by the advisory units — `coding-police`, `comment-police`,
`prose-police` and `format-police` on the Claude side, `version-police` on the OpenCode side. The refusing units do not
