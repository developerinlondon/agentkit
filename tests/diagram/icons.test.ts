...
describe('icon reference expansion', () => {
  test('a set-qualified reference expands to a real path', () => {
    const { source, count } = expandIconRefs('a: X {\n  icon: @logos:postgresql\n}\n');
    expect(count).toBe(1);
    expect(source).toContain(join(assetsDir, 'logos/postgresql.svg'));
  ...
  test('an already-expanded path is left alone', () => {
    const src = 'a: X {\n  icon: ./local.svg\n}\n';
    expect(expandIconRefs(src)).toEqual({ source: src, count: 0 });
  });
