...
function cssRules(html: string): Rule[] {
...
describe('figures are fitted to the screen, never stretched to the column', () => {
  ...
  for (const theme of [['doc', doc], ['deck', deck]] as const) {
    ...
    test(`${name} lifts the height cap on paper and in full screen`, () => {
      // Paper does not scroll and full screen is where a reader goes to escape
      // the cap, so a 60vh figure in either place is the bug, not the fix.
      const printed = only(css, 'figure print', ({ body }) => body.includes('print-color-adjust: exact'));
      expect({ name, print: parse(printed.body)['max-height'] }).toEqual({ name, print: 'none' });
      for (const arm of VIEWPORT_ARMS) {
        expect({ name, arm, printed: printed.selectors.includes(arm) }).toEqual({ name, arm, printed: true });
      }
...
if (!chrome) {
  console.error(
    'SKIPPED the figure-fit measurements in tests/publish-page/theme-chrome.test.ts: no '
...
const ACT_PROBE = `(() => {
  ...
  const out = { fit: box(svg), pageWidth: document.documentElement.scrollWidth };
  tool('in').click();
  tool('in').click();
...
async function measure(template: 'doc' | 'deck'): Promise<Measured> {
  ...
  try {
    ...
    return { wide, narrow, acted, errors: session.logs };
  } finally {
    session?.close();
    await launch.close();
  }
