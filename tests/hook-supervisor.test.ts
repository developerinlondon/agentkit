...
sleep 5`,
  ...
  test('keeps the child deadline below both registered host deadlines', () => {
    ...
    const plugin = JSON.parse(
      readFileSync(join(ROOT, 'plugins-cc', 'agentkit-adversarial-review', 'hooks', 'hooks.json'), 'utf-8'),
    );
    ...
    expect(readFileSync(join(ROOT, 'plugins-cc', 'agentkit', 'hooks', 'hooks.json'), 'utf-8'))
      .not.toContain('fail-closed-hook.sh');
  });
});
