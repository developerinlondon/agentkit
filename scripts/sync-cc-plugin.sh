...
BRAIN_PLUGIN_DIR="$REPO_DIR/plugins-cc/$(kit_plugin_id brain)"
review_hook_script() {
	case "$(basename "$1")" in
...
mkdir -p "$REVIEW_PLUGIN_DIR/hooks" "$REVIEW_PLUGIN_DIR/tools" "$BRAIN_PLUGIN_DIR/hooks"
for hook in "$REPO_DIR"/hooks/claude/*.sh; do
	...
	elif brain_hook_script "$hook"; then
		cp "$hook" "$BRAIN_PLUGIN_DIR/hooks/$(basename "$hook")"
		rm -f "$PLUGIN_DIR/hooks/$(basename "$hook")"
	else
...
if [[ -d "$REPO_DIR/hooks/claude/lib" ]]; then
	mkdir -p "$PLUGIN_DIR/hooks/lib" "$REVIEW_PLUGIN_DIR/hooks/lib" "$BRAIN_PLUGIN_DIR/hooks/lib"
	cp -a "$REPO_DIR"/hooks/claude/lib/. "$PLUGIN_DIR/hooks/lib/"
	cp -a "$REPO_DIR"/hooks/claude/lib/. "$REVIEW_PLUGIN_DIR/hooks/lib/"
	cp -a "$REPO_DIR"/hooks/claude/lib/. "$BRAIN_PLUGIN_DIR/hooks/lib/"
...
for tool in bounded-run plan-gate wait-for wiki-editor wip; do
