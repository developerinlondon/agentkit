...
| **taste-police.sh**    | PreToolUse        | Refuses a command matching one of your own tastes at `enforce: block`, using that taste's `remedy` and its named override. The rules are your files, not this hook: it reads `.agentkit/tastes/`, `.agentkit/tastes-vendor/` and `~/.agentkit/tastes/`, resolves them project > external > user, and enforces nothing when `taste.enabled` is false. Needs `bun`; without it, it reports UNCHECKED rather than allowing quietly                                                                                                                             |
