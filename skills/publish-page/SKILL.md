...
| Loop-back flow, or a fan-out tree, over the sketch register's ~25-text budget          | `diagram` skill split into two figures — mermaid's ELK layout fixes both classes but cannot be inlined here (~1.6 MB of elkjs on top of the mermaid runtime, against ~1.4 MB left in the cap); the ELK recipe is for CDN-loading shells, in the `diagram` skill's `references/selection.md` | dagre mermaid — it strands the loop-back's entry node and tangles a tree's ranks                                                                                                     |
| Everything else — structural, behavioral, data, deployment, or a structural comparison | `diagram` skill, type per its `references/selection.md`                                                                                                                                                                                                                                     | mermaid (a gated flow comes out a 3:1 ribbon with its labels stranded mid-edge, and ELK makes the ratio worse), box kits (no backward edge), a table (flattens the structural point) |
